...
## cell 12 start
# -------------------------------------------------------------------
# Plot 1: SHAP-highlighted input ECG  (unchanged in spirit)
# Red = positive SHAP (supports), Blue = negative SHAP (opposes),
# overlaid on the actual input waveform.
# -------------------------------------------------------------------
def plot_shap_highlighted_ecg(sample_np, shap_3d, explained_idx,
    ...
    for i, ax in enumerate(axes):
        ...
        pos = (np.abs(s) >= thr) & (s > 0)
        neg = (np.abs(s) >= thr) & (s < 0)
        ax.plot(time_axis, y, color="black", lw=0.9, alpha=0.95, zorder=1)
        ax.scatter(time_axis[pos], y[pos], color="red",  s=16, zorder=3, edgecolors="none")
        ax.scatter(time_axis[neg], y[neg], color="blue", s=16, zorder=3, edgecolors="none")
