...
# =============================================================================
# CONFIGURATION
# Adjust MODEL_PATHS / DATASET_ROOT / MAPPING_CSV to match your repo layout.
# =============================================================================
...
# Checkpoint paths (repo-relative).
# Each .pt is a state_dict from train/ : torch.save(model.state_dict(), path)
MODEL_PATHS = [
    "checkpoints/lightv2_ft_fold0.pt",
    "checkpoints/lightv2_p1_fold1.pt",
    "checkpoints/lightv2_p1_fold2.pt",
]

DATASET_ROOT = "dataset/WFDBRecords"
MAPPING_CSV  = "dataset/ConditionNames_SNOMED-CT.csv"
...
def load_ensemble(model_paths):
    ...
    for path in model_paths:
        ...
        state = torch.load(path, map_location=device, weights_only=True)  # [OPT-7]
    ...
    if int(torch.__version__.split(".")[0]) >= 2:  # [OPT-3]
        try:
...
# The same input ECG with zeroed regions shaded in grey.
# =============================================================================

def plot_ablated_ecg(sample_np, ablate_mask, cls_name,
    ...
    These are the regions SHAP flagged as most important.
    ...
    for i, ax in enumerate(axes):
        ...
        # Grey shading over ablated (zeroed) time regions
        in_region, start_t = False, 0
        for t in range(len(ablate_mask)):
            if ablate_mask[t] and not in_region:
                start_t   = t
                in_region = True
            elif not ablate_mask[t] and in_region:
    ...
    legend_items = [
        ...
        mpatches.Patch(facecolor="grey", alpha=0.45,
                       label="Ablated region (top 10% |SHAP| — zeroed to 0)"),
    ]
    fig.legend(handles=legend_items, loc="upper right",
               bbox_to_anchor=(0.98, 0.99), frameon=True, fontsize=9)
    fig.suptitle(
...
def run_ablation(sample_np, ensemble_model, shap_3d, explained_idx,
    ...
    """
    Zero out the top-(100-ablate_percentile)% |SHAP| time steps across all 12
    leads of the INPUT ECG and measure the resulting confidence drop.

    The ablation is applied to the same ECG that was explained — not a
    different file — so the drop directly validates the SHAP map.

    Returns
    -------
    delta       : float       — probability drop (positive = confidence fell)
    ablate_mask : (5000,) bool — which time steps were zeroed
    """
    cls_idx   = explained_idx[explained_rank]
    cls_name  = class_names[cls_idx]
    ...
    attr        = np.abs(sv).mean(axis=0)          # average over leads -> (5000,)
    thr         = np.percentile(attr, ablate_percentile)
    ablate_mask = attr >= thr                      # top 10% time steps

    # Zero out those time steps in the input ECG
    ablated_np              = sample_np.copy()
    ablated_np[:, ablate_mask] = 0.0               # 0 = z-score mean
    ...
    print(f"    Time steps zeroed    : {ablate_mask.sum()} / {SIGNAL_LEN} "
          f"(top {100-ablate_percentile:.0f}% |SHAP|)")
    if delta > 0.05:
    ...
    # Plot the input ECG with grey shading over the zeroed regions
    plot_ablated_ecg(
        sample_np, ablate_mask, cls_name,
        orig_prob, ablated_prob,
        time_axis, record_name, output_dir=output_dir
    )

    return delta, ablate_mask
...
def main():
    ...
    # shap_3d[rank, lead, time] = SHAP attribution for that position
    # entirely derived from the input ECG's gradients
    shap_3d       = np.stack([np.array(v)[0] for v in values], axis=0)
    explained_idx = indexes[0].cpu().numpy().astype(int)
